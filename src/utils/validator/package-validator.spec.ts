...
describe('Package Validator Test', () => {
  ...
  it('should throw an error when validteItemLimit throws', () => {
    const { sut, packageValidationStub } = makeSut()

    const fakeError = new Error('Fake Error')

    jest.spyOn(packageValidationStub, 'validteItemLimit')
      .mockImplementation(() => {
        throw fakeError
      })
  ...
  it('should throw an error when validatePriceAndWeight throws', () => {
    const { sut, packageValidationStub } = makeSut()

    const fakeError = new Error('Fake Error')

    jest.spyOn(packageValidationStub, 'validatePriceAndWeight')
      .mockImplementation(() => {
        throw fakeError
      })
